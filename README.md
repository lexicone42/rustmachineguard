...
A built-in **threat catalog** (84 entries) flags known-malicious or known-vulnerable
...
packages in their own right. See
...
- **Live MCP probing** (`--probe-mcp`) — enumerates each stdio server's tools/resources and scans tool **and parameter** descriptions, plus the server's `instructions` (which the host splices into the model's system prompt), for prompt-injection / line-jumping and invisible-Unicode smuggling. Detects the server's **protocol era**: MCP revision 2026-07-28 removed the `initialize` handshake, so the probe leads with `server/discover` and falls back to `initialize` only on a non-spec error or silence — otherwise a spec-current server would reject `initialize` and be reported as clean.
